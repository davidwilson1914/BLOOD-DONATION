...
load_dotenv()
# Resend API Key-ஐ .env கோப்பிலிருந்து பெறுகிறோம்
...
class NotificationService:
    ...
    # புதிய மின்னஞ்சல் அனுப்பும் முறையை இங்கு சேர்க்கவும்
    def send_email_alert(self, donor_email: str, blood_group: str, location: str, phone: str):
        try:
            response = resend.Emails.send({
                "from": "BloodPulse <onboarding@resend.dev>",
                "to": [donor_email],
                "subject": f"URGENT: {blood_group} Blood Required!",
                "html": f"""
                    <h2>Urgent Blood Request Alert</h2>
                    <p><b>Blood Group:</b> {blood_group}</p>
                    <p><b>Location:</b> {location}</p>
                    <p><b>Contact Phone:</b> {phone}</p>
                    <p>Please log in to respond if you can donate.</p>
                """
            })
