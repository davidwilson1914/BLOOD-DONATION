...
class DatabaseStore:
    ...
    def _seed_initial_data(self):
        ...
        camp1 = Campaign(
            ...
            location=Location(latitude=40.7890, longitude=-73.9548, address="1468 Madison Ave", city="New York"),
            start_date=f"{today.strftime('%d/%m/%Y')} (09:00 AM)",
            end_date=f"{(today + timedelta(days=7)).strftime('%d/%m/%Y')} (05:00 PM)",
        ...
        camp2 = Campaign(
            ...
            location=Location(latitude=40.7391, longitude=-73.9754, address="462 1st Ave", city="New York"),
            start_date=f"{(today - timedelta(days=1)).strftime('%d/%m/%Y')} (08:00 AM)",
            end_date=f"{(today + timedelta(days=5)).strftime('%d/%m/%Y')} (06:00 PM)",
